...
from tongverselite.sensor import Sensor
from tongverselite.tasks.base_task import BaseTask
from tongverselite.utils import Falling, PositionComparator, TimeOut
...
class TaskSix(BaseTask):
    ...
    def step(self, action: Dict) -> Dict:
        ...
        if self._current_sim_step % 30 == 0:
            ...
            tv.sim.step(render=False)
            obs = self._get_obs()

        obs[
            "goal"
        ] = f'Please put {self._target_object_name} \
            on the {TERMINATION_CFG["container"]}'
        obs["pick"] = self._pick
