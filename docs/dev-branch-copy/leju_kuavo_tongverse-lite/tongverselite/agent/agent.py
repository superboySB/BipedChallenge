...
from tongverselite.utils import convert_data_format

from .agent_cfg import KuavoCfg

AGENT_USD_PATH = str(tv.ROOT_PATH.parent / "data" / "agent" / "main.usd")
...
class Agent(ArticulationView):
    ...
    def __init__(
        ...
        if tv.sim.instance() is not None:
            self._backend = tv.sim.instance().backend
            self._device = tv.sim.instance().device
            self._backend_utils = tv.sim.instance().backend_utils
        else:
            self._backend = "torch"
            self._device = None
    ...
    def _process_info_cfg(self) -> None:
        ...
        # self._leg_index = self.cfg.meta_info.leg_index
